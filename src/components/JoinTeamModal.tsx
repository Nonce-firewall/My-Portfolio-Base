...
const JoinTeamModal: React.FC<JoinTeamModalProps> = ({ isOpen, onClose }) => {
  ...
  }, [countdown, showSuccessModal, onClose])
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const submissionData = { ...formData, subject: 'Join Team Application', role: selectedRole, experience: selectedExperience }
  ...
  return createPortal(
    <>
          ...
          {showSuccessModal ? (
            ...
            <div className="p-6 sm:p-8 text-center">
              <div className="relative mx-auto mb-6 w-16 h-16 sm:w-20 sm:h-20">
                <div className="w-full h-full bg-gradient-to-r from-green-400 to-green-600 rounded-full flex items-center justify-center mx-auto animate-scale-in">
            ...
            <>
              <div className="sticky top-0 bg-white border-b border-gray-200 p-4 sm:p-6 rounded-t-2xl z-10">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={onClose} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full transition-all duration-200" aria-label="Close modal">
                    <X size={24} />
                  </button>
                </div>
                <p className="text-gray-600 mt-2 text-sm sm:text-base">
                  We're always looking for talented individuals to join our team. Fill out the form below!
                </p>
              </div>
              <div className="p-4 sm:p-6">
                ...
                <form onSubmit={onSubmit} className="space-y-4 sm:space-y-6">
                  ...
                  <div>
                    ...
                    <button type="button" onClick={() => setShowRoleModal(true)} className="w-full px-4 py-2 border rounded-lg flex justify-between items-center hover:bg-gray-50 text-left" aria-haspopup="listbox" aria-expanded={showRoleModal}>
                      <span className={selectedRole ? 'text-gray-900' : 'text-gray-400'}>{selectedRole || 'Select your desired role'}</span>
                      <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                    </button>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Experience Level *</label>
                    <button type="button" onClick={() => setShowExperienceModal(true)} className="w-full px-4 py-2 border rounded-lg flex justify-between items-center hover:bg-gray-50 text-left" aria-haspopup="listbox" aria-expanded={showExperienceModal}>
                      <span className={selectedExperience ? 'text-gray-900' : 'text-gray-400'}>{selectedExperience || 'Select your experience level'}</span>
                      <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                    </button>
                  </div>
                  <div>
                    <label htmlFor="portfolio" className="block text-sm font-medium text-gray-700 mb-2">Portfolio/LinkedIn URL</label>
                    <input type="url" id="portfolio" name="portfolio" value={formData.portfolio} onChange={handleChange} className="w-full px-3 py-2 sm:px-4 sm:py-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:bg-white transition-all duration-300 text-sm sm:text-base" placeholder="https://yourportfolio.com" />
                  </div>
                  <div>
                    <label htmlFor="message" className="block text-sm font-medium text-gray-700 mb-2">Why do you want to join our team? *</label>
                    <textarea id="message" name="message" required rows={4} value={formData.message} onChange={handleChange} className="w-full px-3 py-2 sm:px-4 sm:py-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:bg-white transition-all duration-300 resize-none text-sm sm:text-base" placeholder="Tell us about yourself..." />
                  </div>
                  <button type="submit" disabled={state.submitting} className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-6 py-3 rounded-xl font-medium transition-all duration-300 hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center transform hover:scale-105 active:scale-95">
                    {state.submitting ? (<><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2"></div><span>Submitting...</span></>) : (<><Send size={20} className="mr-2" />Submit Application</>)}
                  </button>
                </form>
              </div>
            </>
      ...
      {/* These helper modals for selecting roles/experience are unchanged */}
            ...
            <div className="p-4 sm:p-6">
              ...
              <div className="max-h-60 overflow-y-auto space-y-1">
                {roleOptions.map(option => (
                  <button key={option} onClick={() => handleRoleSelect(option)} className={`w-full text-left px-3 py-2 rounded-lg transition-all duration-200 text-sm ${selectedRole === option ? 'bg-blue-100 text-blue-700 font-medium' : 'hover:bg-gray-100 text-gray-700'}`} role="option" aria-selected={selectedRole === option}>
                    {option}
                  </button>
                ))}
              </div>
            ...
            <div className="p-4 sm:p-6">
              ...
              <div className="max-h-60 overflow-y-auto space-y-1">
                {experienceOptions.map(option => (
                  <button key={option} onClick={() => handleExperienceSelect(option)} className={`w-full text-left px-3 py-2 rounded-lg transition-all duration-200 text-sm ${selectedExperience === option ? 'bg-blue-100 text-blue-700 font-medium' : 'hover:bg-gray-100 text-gray-700'}`} role="option" aria-selected={selectedExperience === option}>
                    {option}
                  </button>
                ))}
              </div>
